...
export default function NoticeCard({ notice }: { notice: Notice }) {
  ...
  // 1. Fix: Cast 'a' to any to bypass missing 'mimeType' in strict interface
  ...
  // 2. Fix: Handle both 'category' (singular in Type) and 'categories' (plural in DB) safely
  // We cast to 'any' to bypass the specific TS error and ensure runtime safety
      ...
      <div className="p-5 flex flex-col h-full">
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="flex flex-wrap gap-2">
            ...
            {/* ✅ Fix: Use the normalized displayCategories array */}
            {displayCategories.map((cat: string, idx: number) => (
              <span
                key={idx}
                className="px-2 py-0.5 rounded text-[10px] font-black uppercase tracking-widest border bg-slate-50 text-slate-600 border-slate-200"
              >
                {cat}
              </span>
            ))}
        ...
        <div className="pt-3 border-t border-slate-100 flex items-center justify-between text-xs font-bold text-slate-500">
          ...
          <div className="flex items-center gap-1 text-indigo-600 opacity-0 group-hover:opacity-100 transition-all translate-x-2 group-hover:translate-x-0 duration-300">
            Read <FiExternalLink />
          </div>
        </div>
      </div>
