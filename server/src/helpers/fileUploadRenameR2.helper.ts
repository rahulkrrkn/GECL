...
async function getUniqueKey(params: {
  ...
  while (true) {
    ...
    attempt++;
    if (attempt > 500) {
      throw new Error("Too many duplicate names in storage");
    }
  }
}
...
export async function renameToR2(params: {
  ...
  const filesByField = params.req.files as Record<string, UploadedFile[]>;
  ...
  for (const [fieldName, filesMaybe] of Object.entries(filesByField)) {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      if (optimized.ext) {
        ext = optimized.ext; // overwrite extension only for images
      }
